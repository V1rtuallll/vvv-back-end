...
- **提交与推送遵从 `CICD规范.md`。** 尤其是：**大版本分支**（如 `V1rtualSS`）对应 `../vvv` 的
  前端样式分支（如 `V1rtualSS_sky`），联动改动两端 CI 都绿才算可上线；只推一边不能视为完成。
...
| 环境 | 版本 |
| --- | --- |
| 本地开发库 | MySQL **9.6.0**（Homebrew，2026-09-17 用 `mysql --version` 实测） |
| **生产库** | MySQL **5.6.50** |

本机能跑通不代表生产能跑。**不要使用 MySQL 8 才有的特性**，详见「已知陷阱」。

本机版本跟着 Homebrew 走、会自己往上跳（这一格曾长期写着 8.0.40，实际早已不是）。
但判定标准永远是生产那个 5.6.50，本机版本号只决定「哪些差异本机验不出来」，
所以这里不必追着 brew 更新，改用前跑一次 `mysql --version` 即可。
...
- **生产是 MySQL 5.6，写 SQL 和建表语句都不能用 8 才有的特性。** 本地开发库是 Homebrew 的
  新版 MySQL（2026-09-17 实测 9.6.0，会随 `brew upgrade` 继续往上走），
